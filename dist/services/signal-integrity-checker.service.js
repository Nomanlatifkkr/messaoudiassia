...
exports.SignalIntegrityCheckerService = void 0;
const uuid_1 = require("../utils/uuid");
class SignalIntegrityCheckerService {
    process(input) {
        ...
        if (!input.signalType)
            issues.push('missing signalType');
        if (!input.signalCategory)
            issues.push('missing signalCategory');
        if (!input.signalSource)
            issues.push('missing signalSource');
        if (input.priorityLevel < 1 || input.priorityLevel > 10)
            issues.push('priority out of range');
        if (isNaN(Date.parse(input.timestamp)))
            issues.push('invalid timestamp');
        if (input.version < 0)
            issues.push('negative version');
...
exports.SignalIntegrityCheckerService = SignalIntegrityCheckerService;
